...
select post_id from wp_postmeta where meta_key = '_category_permalink' and meta_value in (200,9,10,419,418,276,99);
<small class="detailGridList">


select GROUP_CONCAT(post_id,'') as cols from wp_postmeta where meta_key = '_category_permalink' and meta_value in (424);
select GROUP_CONCAT(post_id,'') from wp_postmeta where meta_key = '_category_permalink' and meta_value in (200,9,10,419,418,276,99);
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_saude_beleza` AS select `a`.`ID` AS `ID`,`a`.`post_content` AS `post_content`,`a`.`post_title` AS `post_title`,`a`.`post_excerpt` AS `post_excerpt`,(select `wp_postmeta`.`meta_value` AS `state` from `wp_postmeta` where ((`wp_postmeta`.`post_id` = `a`.`ID`) and (`wp_postmeta`.`meta_key` = 'vcard_address'))) AS `endereco`,(select `wp_postmeta`.`meta_value` AS `state` from `wp_postmeta` where ((`wp_postmeta`.`post_id` = `a`.`ID`) and (`wp_postmeta`.`meta_key` = 'vcard_tel'))) AS `telefone`,(select `wp_postmeta`.`meta_value` AS `state` from `wp_postmeta` where ((`wp_postmeta`.`post_id` = `a`.`ID`) and (`wp_postmeta`.`meta_key` = 'vcard_locality'))) AS `cidade`,(select `wp_postmeta`.`meta_value` AS `state` from `wp_postmeta` where ((`wp_postmeta`.`post_id` = `a`.`ID`) and (`wp_postmeta`.`meta_key` = 'vcard_email'))) AS `email`,(select `wp_posts`.`guid` from `wp_posts` where ((`wp_posts`.`post_parent` = `a`.`ID`) and (`wp_posts`.`post_type` = 'attachment') and (`wp_posts`.`guid` is not null) and (`wp_posts`.`guid` like 'http%')) limit 1) AS `logo` from `wp_posts` `a` where (`a`.`ID` in (select `wp_term_relationships`.`object_id` from `wp_term_relationships` where `wp_term_relationships`.`term_taxonomy_id` in (select `wp_term_taxonomy`.`term_id` from `wp_term_taxonomy` where `wp_term_taxonomy`.`term_id` in (select `wp_terms`.`term_id` from `wp_terms` where (term_id = 92)))) and (`a`.`post_type` = 'anuncio') and (`a`.`post_status` = 'publish') and ((select `meta`.`meta_value` from `wp_postmeta` `meta` where ((`meta`.`post_id` = `a`.`ID`) and (`meta`.`meta_key` = 'tipo_anuncio'))) < 1));