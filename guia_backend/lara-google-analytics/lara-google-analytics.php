...
function lrgawidget_enqueue($hook) {
    // var_dump($hook);
    /**
     * If its current home page shows and if user has some event to display data
     */
    if ('index.php' != $hook || !get_user_option("wp_my_events", get_current_user_id())) {
        return;
    }

    // $query = "SELECT count(ID) FROM wp_posts where post_type in ('evento','negocio')";
