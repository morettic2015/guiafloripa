...
class GoogleAnalyticsAPI {
    ...
    public function getAccessToken() {
        ...
        if (!empty($results['access_token']) && !empty($results['token_type']) && !empty($results['expires_in']) && !empty($results['refresh_token'])) {
            ...
            Common\ErrorHandler::FatalError("Invalid Reply", "Google Replied with unexpected replay, enable debugging to check the reply..", 100, $results);
        }
    }
    ...
    public function refreshAccessToken() {
        ...
        if (!empty($results['access_token']) && !empty($results['token_type']) && !empty($results['expires_in'])) {
            ...
            Common\ErrorHandler::FatalError("Invalid Reply", "Google Replied with unexpected replay, enable debugging to check the reply..", 101, $results);
        }
    }
