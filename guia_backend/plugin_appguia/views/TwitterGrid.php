 ...
 * @author  Matt van Andel
 */
class TwitterGrid extends WP_List_Table {
    ...
    public function get_columns() {
        $columns = array(
            ...
            'title' => _x('Hashtag', 'Hashtag de busca', 'wp-list-table-example'),
            'rating' => _x('Favoritos', 'Adicionar aos favoritos', 'wp-list-table-example'),
            'follow' => _x('Seguir', 'Seguir autor', 'wp-list-table-example'),
            'director' => _x('Retweetar', 'Retwittar post', 'wp-list-table-example'),
            'quote' => _x('Menção', 'Menção no retweet', 'wp-list-table-example'),
            'ignore' => _x('Ignorar?', 'Ignorar tweet', 'wp-list-table-example'),
        );
    ...
    protected function column_title($item) {
        ...
        $edit_query_args = array(
            'page' => "app_guiafloripa_twitter_add_term",
            'action' => 'edit',
            'term' => $item['ID'],
        );
    ...
    protected function process_bulk_action() {
        ...
        if ('delete' === $this->current_action()) {
            $query = "delete FROM app_guiafloripa_com_br.wp_usermeta where user_id = ".get_current_user_id()." and umeta_id ";
            if (is_array($_GET['term'])) {
                foreach ($_GET['term'] as $row) {
                    $id .= $row . ",";
                }
                $query .= ' in (' . $id . '0)';
            ...
            echo '<div class="notice notice-success is-dismissible"> 
                    <p><strong>Hashtag(s) de busca removida(s) com sucesso ID(s):' . $id . '.</strong></p>
                 </div>';
            echo '<a href="admin.php?page=app_guiafloripa_twitter" class="page-title-action">Voltar</a></h1>';
            wp_die('Sucesso!');
        }
    }
    ...
    function prepare_items() {
        ...
        foreach ($twitterMeta as $t) {
            ...
            $fav = empty($obj->favoritos) ? "NÃO" : "SIM";
            $ret = empty($obj->retweetar) ? "NÃO" : "SIM";
            $quo = empty($obj->quote) ? "" : $obj->quote;
            $fol = empty($obj->follow) ? "NÃO" : "SIM";
            $ign = isset($obj->ignore) ? "SIM" : "NÃO";
