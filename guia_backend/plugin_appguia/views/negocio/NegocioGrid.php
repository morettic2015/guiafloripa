...
class NegocioGrid extends WP_List_Table {
    ...
    public function get_columns() {
        $columns = array(
            ...
            'director' => _x('Logo', 'Column label', 'wp-list-table-example'),
            'title' => _x('Titulo', 'Titulo', 'wp-list-table-example'),
            'rating' => _x('Categoria', 'Column label', 'wp-list-table-example'),
    ...
    protected function column_title($item) {
        ...
        $actions['general'] = '<a href="admin.php?page=app_guiafloripa_negocio_add&id=' . $item['ID'] . ' ">' . _x('Editar') . '</a>';
    ...
    protected function process_bulk_action() {
        ...
        if ('delete' === $this->current_action()) {
            ...
            $campaign .= "-1";
            $query = "delete FROM wp_posts where post_type = 'business' and post_author = " . get_current_user_id() . " and ID in ($campaign)";
    ...
    function prepare_items() {
        ...
        foreach ($cp as $t) {
            ...
            //var_dump($meta);
            // die();
            $term = get_term($meta['business_type'][0], 'business_type');
            // var_dump($term);
            $vet[] = array(
                'ID' => $t->ID,
                'title' => $t->post_title,
