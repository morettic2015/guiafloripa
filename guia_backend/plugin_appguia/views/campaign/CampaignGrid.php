...
class TT_Example_List_Table extends WP_List_Table {
    ...
    protected function column_title($item) {
        ...
        $actions['edit'] = '<a href="javascript:showPop(\'place\',' . $item['ID'] . ')">' . _x('Editar') . '</a>';
        $actions['dates'] = '<a href="javascript:showPop(\'dates\',' . $item['ID'] . ')">' . _x('Relatorio') . '</a>';
    ...
    protected function process_bulk_action() {
        ...
        if ('delete' === $this->current_action()) {
            ...
            $campaign .= "-1";
            $query = "delete FROM wp_posts where post_type = 'campaign' and post_author = " . get_current_user_id() . " and ID in ($campaign)";
            $cp = $wpdb->get_results($query);
            echo '<div class="notice notice-success is-dismissible"> 
                    <p><strong>Sucesso. As campanhas selecionadas foram excluidas.</strong></p>
                 </div>';
        }
    }
    ...
    function prepare_items() {
        ...
        $query = "SELECT * FROM wp_posts where post_type = 'campaign' and post_author = " . get_current_user_id();   // var_dump($wpdb);
        // echo $query;
        $cp = $wpdb->get_results($query);
